...
class RolloutEncoder(nn.Module):
    def __init__(self, input_shape, hidden_size=ROLLOUT_HIDDEN):
        ...
        self.lstm = nn.LSTMCell(input_size=conv_out_size+2, hidden_size=hidden_size)
    ...
    def forward(self, obs_v, reward_v):
        ...
        _, hidden, ponder_dict = self.lstm_act(rnn_in)
        return hidden.view(-1), ponder_dict
...
class I2A(nn.Module):
    ...
    def forward(self, x):
        fx = x.float() / 255
        enc_rollouts, ponder_dict = self.rollouts_batch(fx)
        conv_out = self.conv(fx).view(fx.size()[0], -1)
        fc_in = torch.cat((conv_out, enc_rollouts), dim=1)
        fc_out = self.fc(fc_in)
        return self.policy(fc_out), self.value(fc_out), ponder_dict
    ...
    def rollouts_batch(self, batch):
        ...
        flat_enc_v, ponder_dict = self.encoder(step_obs_v, step_rewards_v)
        return flat_enc_v.view(batch_size, -1), ponder_dict
