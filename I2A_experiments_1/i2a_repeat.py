...
class RolloutEncoder(nn.Module):
    def __init__(self, input_shape, hidden_size=ROLLOUT_HIDDEN):
        ...
        self.rnn = nn.LSTM(input_size=conv_out_size+2, hidden_size=hidden_size, batch_first=False)

    def _get_conv_out(self, shape):
        o = self.conv(torch.zeros(1, *shape))
        return int(np.prod(o.size()))

    def forward(self, obs_v, reward_v, batch_size, rollout_steps):
        """
        ...
        binary_flag = np.zeros((rollout_steps, PONDER*n_batch, 1))
        ...
        _, (rnn_hid, _) = self.rnn(rnn_input)
...
class I2A(nn.Module):
    def __init__(self, input_shape, n_actions, net_em, net_policy, rollout_steps):
        ...
        conv_out_size = self._get_conv_out(input_shape)
        fc_input = conv_out_size + ROLLOUT_HIDDEN * n_actions * PONDER
